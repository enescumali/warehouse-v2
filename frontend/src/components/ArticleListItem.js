...
function ArticleListItem({ article, onStatusChange, substractArticles }) {
  ...
  if (substractError) {
    return (
      <ErrorMessage
        className="mb-10"
        dataTestId="errorMessage"
        onErrorAction={() => setTrySubsctractAgain(true)}
        message={substractError}
      ></ErrorMessage>
    );
  }
  ...
  return (
    <div className="mb-6">
      <p className="font-bold" data-testid="articleName">
        {articleDetail.name}
      </p>
      <span className="font-light text-xs mr-2">
        {articleDetail.amountInStock > 0 ? (
          <>
            ...
            <span data-testid="articleAmount">
              In stock ({articleDetail.amountInStock})
            </span>
          </>
        ) : (
          <>
      ...
      <span className="font-light italic text-xs" data-testid="articleRequired">
        {" "}
        Required: {article.amountRequired}
      </span>
    </div>
