    ...
    fetch(`${CONSTANTS.apiUrl}/sales`, requestOptions)
      .then((response) => {
        if (response.ok) {
          ...
          throw new Error(
            "Something went wrong creating the sale, we are on it!"
          );
        }
      })
      .then((result) => {
  ...
  return (
    <div>
      ...
      <div className="items-center md:flex">
        <button
          className={
            "justify-center mb-2 md:mb-0 md:mr-2 primary-button sm:w-auto w-full " +
            (isLoading ? "loading" : "")
          }
          ...
          <div>
            {isSaleSuccessful ? (
              <p className="text-green-900 text-sm">
                Sale created succesfully! Product list will be updated
                accordingly...{" "}
              </p>
            ) : (
              <ErrorMessage
                message={error}
                onErrorAction={() => createSale(product.id)}
              ></ErrorMessage>
            )}
          </div>
        ) : (
